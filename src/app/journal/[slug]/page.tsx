    ...
    <>
      ...
      <main className="pt-20">
        <section className="relative h-[644px] bg-[#1C1A17] overflow-hidden">
          ...
          <div className="absolute bottom-20 left-1/2 -translate-x-1/2 w-[860px] text-center">
            <p className="font-inter font-medium text-[12px] tracking-[4px] uppercase text-[#AFD9F2] mb-6">職人誌 · Artisan Journal</p>
            <h1 className="font-shippori text-[52px] tracking-[-0.5px] leading-[1.2] text-white">{title}</h1>
          </div>
        </section>
          ...
          <div className="grid grid-cols-3 gap-8">
            {relatedArticles.length > 0 ? relatedArticles.map((a) => (
              <Link key={a.id} href={`/journal/${a.slug}`} className="group">
                ...
                <h3 className="font-shippori text-[22px] leading-[1.4] text-[#01002D] group-hover:text-[#030074] transition-colors">{a.title}</h3>
              </Link>
